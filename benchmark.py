...
Both backends now time the SAME full pipeline by default (pil_to_tensor
-> resize -> model forward -> normalize -> PIL resize-back), matching
`DepthAnythingMLX.estimate()`'s own contract exactly. This wasn't always
true: earlier versions of this script only timed real-torch's bare
`model(**inputs)` call, with preprocessing computed once outside the
loop and no resize-back at all -- excluding ~274ms (pil_to_tensor) +
~182ms (PIL resize-back) at 108MP that torch-mlx's own number always
included. Every real-torch number produced before this fix (including
some already written into BENCHMARK_RESULTS.md, corrected in place
rather than silently) was not comparable to torch-mlx's. Pass
`--bare-model-only` to reproduce that old (misleading) behavior.
...
def _run(python_bin: str, backend: str, iters: int, warmup: int, device: str | None = None, compiled: bool = False, image_size: tuple[int, int] = (480, 640), dtype: str | None = None, full_pipeline: bool = False) -> dict:
...
def main() -> None:
    ...
    parser.add_argument(
        "--bare-model-only", action="store_true",
        help="real-torch: time only model(**inputs) with preprocessing done once outside the loop and no "
             "resize-back to the original image size -- the ORIGINAL (misleading) behavior of this script. "
             "Found via profiling: this excludes ~274ms (pil_to_tensor) + ~182ms (PIL resize-back) at 108MP "
             "that torch-mlx's own estimate() timing always included, making every real-torch number in this "
             "file's history before this flag existed not comparable to torch-mlx's. Default is now the fair "
             "full pipeline (same pil_to_tensor + resize + forward + resize-back torch-mlx's estimate() does).",
    )
    args = parser.parse_args()
    size = (args.height, args.width)
    full_pipeline = not args.bare_model_only
    ...
    if args.all:
        for device in ("cpu", "mps"):
            ...
            runs.append((f"real PyTorch ({device})", _run(args.python, "real-torch", args.iters, args.warmup, device, image_size=size, full_pipeline=full_pipeline)))
    else:
        print(f"Running real-torch backend (device={args.real_torch_device})...")
        runs.append((
            f"real PyTorch ({args.real_torch_device})",
            _run(args.python, "real-torch", args.iters, args.warmup, args.real_torch_device, image_size=size, full_pipeline=full_pipeline),
        ))
